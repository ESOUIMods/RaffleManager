...
function RaffleManager_LoadEvents ()
    local function Loader ()
        ...
        CHAT_SYSTEM:AddMessage("Fetching " .. SELECTED_DAY .. " days worth of bank history for " .. SELECTED_GUILD .. "...")
        ...
        if DoesGuildHistoryCategoryHaveMoreEvents(gnum, GUILD_HISTORY_BANK) then
            -- get the oldest event... again

            for i = 1, num_events do
                local etype, secs, par1, par2, par3, par4, par5, par6 = GetGuildEventInfo(gnum, GUILD_HISTORY_BANK, i)
                table.insert(events, secs)
            end

            table.sort(events, function (k1, k2) return k1 < k2 end)

            if events[#events] >= ten_days then
                LOADING_EVENTS = false
                return false
            end

            if not RequestGuildHistoryCategoryNewest(gnum, GUILD_HISTORY_BANK) and not RequestGuildHistoryCategoryOlder(gnum, GUILD_HISTORY_BANK) then
                return false
            end

            LOADING_EVENTS = true

            zo_callLater(Loader, 2000)
        else
            LOADING_EVENTS = false
            return false
        end
...
function RaffleManager_ParseRoster ()
    if SELECTED_GUILD == nil then CHAT_SYSTEM:AddMessage("Select a guild before exporting!") return end
...
local function SendNextRecipient()
	...
	elseif not (mailBoxOpen) then
		...
		zo_callLater(SendNextRecipient, throttleTimer)
		else RAFFLEMANAGER_ACTIVE = false 
			CloseMailbox() 
		end
	end
end
...
local function RaffleManagerCancel()
	...
	else RAFFLEMANAGER_PAUSED = false
...
local function OnMailSendSuccess()
	if (RAFFLEMANAGER_ACTIVE) then
		...
		else d("RaffleManager Completed")
			RAFFLEMANAGER_ACTIVE = false
